...
The current platform names, aliases, source readback, bounded checks, and unresolved release fields
are maintained in [Current Deployment](current-deployment.md). Use the
[Deployment Runbook](deployment-runbook.md) for route selection and release gates. This runtime
procedure owns process shape, migration order, health, and recovery controls; it is not a second
hosted status ledger.
