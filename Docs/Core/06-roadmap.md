...
| 2 | Complete disposable PostgreSQL verification for v0.1 and standing v0.2. | Run migrations, focused suites, race/restart suites, and pinned conformance on a named non-runtime database. |
| 3 | Decide whether and how to expose the standing control plane. | Accept lifetime, renewal, redaction, CSRF, token custody, login continuation, revocation, and snapshot semantics before public implementation. |
...
Do not add speculative transports, hidden compatibility fallbacks, multi-instance ownership, public
Grant controls, quota policy, consumer-specific business rules, or production claims merely to make a
roadmap look complete. Each would require a separate accepted contract and evidence plan.
