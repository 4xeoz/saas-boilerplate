...
|---|---|---|
| 1 | Integrate the reviewed Re-entry Core source identity used by conformance. | Current-source selection is accepted; exact source, shared standing scenario, fresh-process recovery, and exact-source upgrade pass on Receiver commit `1ed853b`; compatibility and release remain open. |
| 2 | Integrate the existing PostgreSQL baseline and complete additional handoff coverage. | All 203 backend tests, shared Consent HTTP/persistence coverage, and the exact nine-migration upgrade pass on the integrated Receiver commit; deployment and hosted readback remain open. |
